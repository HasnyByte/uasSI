        ...
        <div class="absolute inset-0 bg-gradient-to-b from-black/50 via-black/30 to-black/20">
            <div class="absolute inset-0 flex flex-col items-center justify-center z-10 text-center">
                ...
                <div class="pt-2 text-[20px]">
                    Various references to mainstay food / culinary in Aceh.
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="w-full lg:w-9/12">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($kuliners as $kuliner)
                            <a href="{{ route('kuliner.show', $kuliner['id_kuliner']) }}" class="block relative h-48 rounded-lg overflow-hidden shadow-md group">
                                <!-- Gambar -->
                                <div class="absolute inset-0 z-0 bg-cover bg-center transition-transform duration-300 group-hover:scale-105" style="background-image: url('{{ $kuliner['foto_kuliner'] }}');"></div>
