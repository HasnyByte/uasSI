...
<div class="relative text-white bg-cover bg-center h-64" style="background-image: url('https://thumb.viva.co.id/media/frontend/thumbs3/2022/04/05/624c501348902-mie-aceh_1265_711.jpg');">
    ...
    <div class="absolute inset-0 bg-gradient-to-b from-black/50 via-black/30 to-black/20">
        <div class="absolute inset-0 flex flex-col items-center justify-center z-10 text-center">
            <div class="font-bold text-[30px]">Aceh Culinary</div>
            <div class="pt-2 text-[20px]">Various references to mainstay food / culinary in Aceh.</div>
        </div>
    </div>
</div>
    ...
    <div class="container mx-auto px-8">
        <div class="flex flex-col lg:flex-row lg:items-center gap-4 mb-6">
            <div class="w-full lg:w-3/12">
                <div class="font-bold text-lg">Eat & Drink</div>
            </div>
                    ...
                    <div class="w-fit max-w-sm ml-auto">
                        <form action="{{ route('kuliner') }}" method="GET">
                            <div class="flex rounded-[12px] shadow-sm overflow-hidden">
                                <input type="text" name="search" placeholder="Cari kuliner..." value="{{ request('search') }}" class="px-4 py-2 border border-gray-300 rounded-l-[12px] focus:outline-none focus:ring-2 focus:ring-[#2A933C]" />
            ...
            <section class="w-full lg:w-9/12">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @forelse($kuliner as $item)
                        ...
                        @endphp
                        <a href="{{ route('kuliner.show', ['id' => $item->id_kuliner]) }}" class="block relative h-48 rounded-lg overflow-hidden shadow-md group">
                            ...
                            <div class="absolute inset-0 z-10 bg-black/40 flex flex-col justify-end p-4">
                                <h3 class="font-semibold text-lg text-white">{{ $item->nama_kuliner }}</h3>
                                <p class="text-xs text-white">ID Debug: {{ $item->id_kuliner }}</p>
                    ...
                    @empty
                        <p class="text-gray-600">Tidak ada kuliner tersedia.</p>
                    @endforelse
                </div>
                ...
                <div class="flex justify-center mt-8">
                    <ul class="flex space-x-2">
                        ...
                        @else
                            <li>
                                <a href="{{ $kuliner->previousPageUrl() }}" class="px-3 py-1 border border-gray-300 text-gray-600 rounded hover:bg-gray-100">Prev</a>
                            </li>
                        @endif

                        {{-- Page Numbers --}}
                        @foreach ($kuliner->getUrlRange(1, $kuliner->lastPage()) as $page => $url)
                            <li>
                        ...
                        @if ($kuliner->hasMorePages())
                            <li>
                                <a href="{{ $kuliner->nextPageUrl() }}" class="px-3 py-1 border border-gray-300 text-gray-600 rounded hover:bg-gray-100">Next</a>
                            </li>
                        @else
